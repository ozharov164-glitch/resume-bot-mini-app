...
async def _send_docx_best_effort(
    ...
    try:
        docx_bytes = generate_docx_bytes(resume_data, template_id)
...
async def download_pdf(resume_id: str, current_user: dict = Depends(get_current_user), db=Depends(get_db)):
    ...
    try:
        await send_document_to_user(
            ...
            document=pdf_bytes,
            filename=filename,
            caption=f"Готово! PDF, DOCX уже в чате. Удачи в поиске работы, {resume_data.get('full_name', '')}.",
        )
    except Exception as exc:
        logger.exception("telegram send failed resume_id=%s telegram_id=%s", resume_id, current_user.get("telegram_id"))
        raise HTTPException(
            status_code=502,
            detail="PDF, DOCX готовы, но не удалось отправить в Telegram. Напишите боту /start и попробуйте снова.",
        ) from exc
